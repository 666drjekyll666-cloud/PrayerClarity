    ...
    internal static class RebalancedRuleSet
    {
        private static readonly Dictionary<string, RebalancedPrayerRule> Rules =
            new Dictionary<string, RebalancedPrayerRule>(StringComparer.Ordinal)
            {
                ...
                ["b_skull"] = new RebalancedPrayerRule("b_skull", requirements: F(20f, 40f, 90f), faithBonusRates: F(0f, 0f, 0f), moneyBonusRates: F(0f, 0f, 0f), removeFixedFaith: true, removeFixedMoney: true, durationMinutes: F(30f, 42f, 54f), reposeModes: new[] { ReposeQualityMode.Stock, ReposeQualityMode.HalfwayToBest, ReposeQualityMode.Best }),
