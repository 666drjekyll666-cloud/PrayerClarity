...
- **Repose player-facing language — candidate Rebalanced 0.2.45 / sibling Vanilla 1.0.51 pending visual acceptance.** Runtime review of 0.2.44 accepted the prayer-item wording structure and the terminal Bronze one-line-per-quality star presentation, but the language was refined once more for immediate reliability clarity. Concrete Bronze now states that the Donkey can bring higher-quality bodies **but the best body is not guaranteed**; Silver/Gold keep the same shared effect plus improved/guaranteed best-body reliability. Technology remains the compact comparison surface. At the progression ceiling, Silver/Gold explicitly refer to the best among the bodies the Donkey can **already** bring, while the false higher-tier sentence remains omitted. Temporary Effects and the shared natural-limit sentence are unchanged from the already-corrected 0.2.43 behavior. Repose mechanics, endpoint predicates, distribution behavior, requirements, duration and RNG remain unchanged. Vanilla does not inherit Rebalanced reliability semantics.

- **Prayer-item parchment balance — candidate Rebalanced 0.2.45 / sibling Vanilla 1.0.51 pending visual acceptance.** Runtime evidence rejected all three PrayerClarity-owned width policies tried so far: fixed 200 was too narrow, ResizeFreely/420 let one longest line expand the whole parchment, and fixed 280 still looked visibly non-native/unbalanced. The selected path is now to **stop owning prayer-item width entirely**. PrayerClarity-owned inserted mechanics rows keep only the accepted conditional final-wrap repair for amount + inline-resource icons; width/height/overflow remain game-owned standard item-tooltip behavior. Technology and non-prayer item tooltips remain unchanged.
