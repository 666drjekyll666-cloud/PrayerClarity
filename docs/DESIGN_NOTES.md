...
- **Repose player-facing language — candidate Rebalanced 0.2.43 / sibling Vanilla 1.0.49 pending visual acceptance.** 0.2.42 established the accepted surface-specific language model but its runtime pass exposed two remaining context errors: terminal Bronze still carried the corpse-quality glyph cluster after the natural-limit sentence, and Character -> Temporary Effects still claimed that the Donkey could bring higher-quality bodies after the ordinary quality ceiling had already been reached. 0.2.43 removes the terminal glyph cluster, renders the Bronze/Silver/Gold explanation as a parenthetical note with explicit prayer names, and makes the active Repose description consume the same accepted corpse-tier semantics as the pulpit. Forecast/planning surfaces still say the Donkey **will be able** to bring higher-quality bodies; non-terminal Temporary Effects says the Donkey **can** bring them; terminal Silver/Gold show only the still-working reliability benefit when it remains meaningful. This is presentation-only: Repose mechanics, endpoint predicates, distribution behavior, requirements, duration and RNG remain unchanged.

- **Prayer-item parchment balance — candidate Rebalanced 0.2.43 / sibling Vanilla 1.0.49 pending visual acceptance.** The shared 0.2.29-era fixed 200-unit mechanics column is now a proved owner for the conspicuous right-side empty strip reported across prayer item tooltips. 0.2.43 returns marked mechanics rows to the earlier native ResizeFreely + finite 420-unit ceiling model while retaining the later accepted amount+inline-resource final-wrap repair. Scope is PrayerClarity-owned prayer-item mechanics rows only; Technology and non-prayer item tooltips are unchanged.
