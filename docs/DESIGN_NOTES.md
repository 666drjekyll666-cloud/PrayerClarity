...
- **Repose player-facing terminology — candidate 0.2.41 pending visual acceptance.** Runtime testing of 0.2.40 confirmed the existing endpoint branch switches correctly between progression states, but its Bronze terminal wording was rejected as too bulky / not immediately self-explanatory. The accepted language model now separates two concepts explicitly: **quality tiers that are available at all** versus **the best body among those already available**. The shared base effect says the donkey can bring bodies from a higher quality tier; Rebalanced Bronze opens the next tier, Silver raises the chance of the best available body, and Gold guarantees the best available body. At the Bronze terminal state, no new tiers can be opened; native corpse-quality and prayer-quality glyphs reinforce the same grammar. This is wording-only: mechanics, endpoint detection, Silver/Gold distribution behavior, layout and Vanilla mechanics remain unchanged. The clearer shared base phrase applies to both sibling editions.
