...
- **Prayer-item 100%-success requirement alignment — candidate Rebalanced 0.2.46 / sibling Vanilla 1.0.52 pending focused visual acceptance.** Standard vanilla width can legitimately wrap the Church Quality number + cross onto a second line in Russian. The text is functionally correct, but left alignment makes that wrap visually accidental. 0.2.46 changes only the PrayerClarity-owned requirement content row from left to centered alignment; Base Result and On Success content remain left-aligned, section headers remain centered, and no width/overflow setting is changed.
