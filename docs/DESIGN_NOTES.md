...
- **Repose player-facing language — candidate Rebalanced 0.2.42 / sibling Vanilla 1.0.48 pending visual acceptance.** 0.2.41 is superseded before acceptance: its shared sentence could still read as a fourth effect, and the tier lines did not clearly communicate that Silver/Gold inherit the same base higher-quality-body effect. The accepted model is now surface-specific while keeping one game-world vocabulary. Forecast/planning surfaces say the Donkey **will be able** to bring higher-quality bodies; Temporary Effects says the Donkey **can** bring them. Rebalanced Technology says the shared effect once, then only the quality delta (Bronze = main effect only; Silver = higher chance of the best available body; Gold = best available body guaranteed). Concrete items and ordinary pulpit selections are self-contained. At the progression ceiling, the shared endpoint says that still-higher-quality bodies can no longer be obtained — even prayer cannot help; terminal Rebalanced Bronze explains Bronze/Silver/Gold in words, while terminal Silver/Gold show only the reliability benefit that still works. Corpse-quality glyphs remain as the body-quality cue; prayer-quality star glyphs are no longer used in the terminal explanatory sentence. This is presentation-only: Repose mechanics, endpoint predicates, distribution behavior, requirements, duration, RNG and layout remain unchanged.
